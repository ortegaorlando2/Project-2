...
let link = "static/js/data.geojson";
...
// Grab the data with d3
  ...
  data.features.forEach(record => {
    ...
    let caseHome = record.features
    homesMap(caseHome)

    function homesMap(caseHome){
      //console.log(caseschool)
      for (let k=0; k<caseHome.length; k++){
        let latitude = parseInt(caseHome.coordinates[1])
        let longitude = parseInt(caseHome.coordinates[0]) 
      dict_k={}
  
      dict_k["latitude"]= latitude
      dict_k["longitude"]= longitude
  ...
  let depth = record.geometry.coordinates[2]/100
    ...
    if (location) {
      ...
      markers.addLayer(L.circleMarker([latitude, longitude],
        {
        ...
        .bindPopup("garages: " + record.properties.garages + "<br>Bathrooms : " + record.properties.total_baths));
      }
    }
  ...
  d3.csv("static/js/all_clean_ratings.csv").then((caseschool)=> schoolRatings(caseschool));

  schDict=[]
  let indexj=[]

  function schoolRatings(caseschool){
    //console.log(caseschool)
    for (let j=0; j<caseschool.length; j++){
    let campus_number=caseschool[j].campus_number
    let district=caseschool[j].district
    let school=caseschool[j].school
    let yrs_unacceptable=caseschool[j].yrs_unacceptable
    let rating = caseschool[j].rating
    let year=caseschool[j].year
    //console.log(campus_number,district,school, yrs_unacceptable,rating,year)

    dict_j={}

    dict_j["campus_number"]= campus_number
    dict_j["district"]= district
    dict_j["school"]= school
    dict_j["yrs_unacceptable"]= yrs_unacceptable
    dict_j["rating"]= rating
    dict_j["year"]= year
